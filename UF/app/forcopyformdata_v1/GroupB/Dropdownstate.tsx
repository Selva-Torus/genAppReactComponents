  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "e386da4bc3384d709df05fbf6a10b6c9",
        isTable: false,
...
if(!dropdownData.COUNTRY && Array.isArray(dropdownData.COUNTRY)){
  ...
  useEffect(() => {
    ...
    getDropdownData()
  },[state0b6c9?.refresh,dropdownData.COUNTRY])

  const handlechange = async(value: any) => {
    if(value.length>0){
      setbfedf0((prev: any) => ({ ...prev, state: value }))
  ...
  useEffect(() => {
    ...
    // setbfedf0((pre:any)=>({...pre,state:dfd_codedescription_v1Props[0]?.state}))
    }
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbfedf0((pre:any)=>({...pre,state:""}))
    else
      setInitialCount(1)
  },[state0b6c9?.refresh])

  if (state0b6c9?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `9 / 11`,gridRow: `15 / 25`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
      </div>
      <Dropdown   
        className=""    
        disabled= {state0b6c9?.isDisabled ? true : false}
        width = "250"
        static={true}
        ...
        value={bfedf0?.state ? [bfedf0?.state] : []}
        />
        {validate?.state && (
