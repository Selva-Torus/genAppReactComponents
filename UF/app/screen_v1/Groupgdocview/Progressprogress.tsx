...
const Progressprogress = ({encryptionFlagCompData}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "b2969084c65b4941b05a79ca7dd3b6ff",
          isTable: false,
    ...
    let temp:any = dfd_userdfd_v1Props[0]?.age
    ...
    setgdocview62373((pre:any)=>({...pre,age:temp}))
    handleCustomCode()
  }
  ...
  useEffect(()=>{
    handleMapperValue()
  },[progress3b6ff?.refresh])

  if (progress3b6ff?.isHidden) {
    return <></>
  }

return (
  <div 
    style={{gridColumn: `2 / 12`,gridRow: `30 / 40`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Progress 
      className="w-full "
        size = {'s'}
        theme = {'success'}
        text = {'sample_text'}
        value = {gdocview62373?.age ||0}
    />
  </div>
  )
}
