  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "18d32f1987d042a7867c421457d7a4d2",
        isTable: false,
...
if(!dropdownData.COUNTRY && Array.isArray(dropdownData.COUNTRY)){
  ...
  useEffect(() => {
    ...
    getDropdownData()
  },[state7a4d2?.refresh,dropdownData.COUNTRY])

  const handlechange = async(value: any) => {
    if(value.length>0){
      seta374af((prev: any) => ({ ...prev, statecode: getMapperDetailsBindValues[value],statevalue: value }))
  ...
  useEffect(() => {
    ...
    // seta374af((pre:any)=>({...pre,statecode:dfd_codedexcriptionv2_v1Props[0]?.statecode}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      seta374af((prev: any) => ({ ...prev, statecode: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       seta374af((prev: any) => ({ ...prev, statecode: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,statecode:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     seta374af((pre:any)=>({...pre,statecode:""}))
    else
      setInitialCount(1)
  },[state7a4d2?.refresh])

  if (state7a4d2?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `8 / 10`,gridRow: `11 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
      </div>
      <Dropdown   
        className=""    
        disabled= {state7a4d2?.isDisabled ? true : false}
        width = "250"
        static={true}
        staticProps={stateOptions}
        placeholder={keyset("STATE")} filterable={true} hasClear={true} onChange={handlechange} value={a374af?.statevalue ? [a374af?.statevalue] : []}
        />
         {validate?.state && (
