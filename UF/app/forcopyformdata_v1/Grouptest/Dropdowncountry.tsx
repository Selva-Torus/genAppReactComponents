...
const Dropdowncountry = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "07adc0224a424c4bb21bd09ca64a7836",
        isTable: false,
  ...
  const handlechange = async(value: any) => {
    if(value.length>0){
      settestc78ea((prev: any) => ({ ...prev, country: value }))
  ...
  useEffect(() => {
    ...
    // settestc78ea((pre:any)=>({...pre,country:dfd_codedescription_v1Props[0]?.country}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      settestc78ea((prev: any) => ({ ...prev, country: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,country:undefined}))
  ...
  const handleBlur = async () => {
      if(testc78ea?.country == "" || testc78ea?.country == undefined){
      testc78ea.country = "";
      const validate:any = v.safeParse(schema, testc78ea?.country);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,country:"invalid"}))
        }
    }else if(testc78ea?.country !== ""){
    const validate:any = v.safeParse(schema, testc78ea?.country);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,country:"invalid"}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     settestc78ea((pre:any)=>({...pre,country:""}))
    else
      setInitialCount(1)
  },[countrya7836?.refresh])

  if (countrya7836?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `2 / 4`,gridRow: `10 / 27`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        {isRequredData && <span style={{ color: 'red' }}>*</span>}
      </div>
      <Dropdown   
        className=""    
        disabled= {countrya7836?.isDisabled ? true : false}
        width = "250"
        static={true}
        ...
        value={testc78ea?.country ? [testc78ea?.country] : []}
        validationState={validate?.country ? "invalid" : undefined}
        errorMessage={error}
        />
        {validate?.country && (
