  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "04e028a22954429d913392758890a02b",
        isTable: false,
...
if(!dropdownData.STATE && Array.isArray(dropdownData.STATE)){
  ...
  useEffect(() => {
    ...
    getDropdownData()
  },[city0a02b?.refresh,dropdownData.STATE])

  const handlechange = async(value: any) => {
    if(value.length>0){
      setbfedf0((prev: any) => ({ ...prev, city: value }))
  ...
  useEffect(() => {
    ...
    // setbfedf0((pre:any)=>({...pre,city:dfd_codedescription_v1Props[0]?.city}))
    }
  ...
  useEffect(() => {
    if(initialCount!=0)
     setbfedf0((pre:any)=>({...pre,city:""}))
    else
      setInitialCount(1)
  },[city0a02b?.refresh])

  if (city0a02b?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `8 / 10`,gridRow: `52 / 62`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
      </div>
      <Dropdown   
        className=""    
        disabled= {city0a02b?.isDisabled ? true : false}
        width = "250"
        static={true}
