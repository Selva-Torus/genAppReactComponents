...
const TextInputname = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    ...
    setbfedf0((prev: any) => ({ ...prev, name: +e.target.value }))
    }
    else{
    setbfedf0((prev: any) => ({ ...prev, name: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "af482f5e18f7404db4c3d1e5eb805359",
          isTable: false,
          from:"TextInputname",
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setbfedf0((pre:any)=>({...pre,name:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (name05359?.isHidden) {
    return <></>
  }
  return (   
    <div 
      style={{gridColumn: `2 / 4`,gridRow: `20 / 30`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TextInput
        ...
        value={bfedf0?.name||""}
         disabled= {name05359?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {name05359?.isDisabled ? true : false}
        size='m'      
        view='normal'
